<template>
  <div class="achat">
    <!-- Hero Section -->
    <HeroSearch :title="pageTitle" :text="pageText" active="achat" bg="achat" :extras="data?.extras || {}"
      :extra="data?.extra || []" />
    ...
    <section v-if="!loading && data" class="featured portfolio bg-gray-50 dark:bg-gray-900 py-8">
        ...
        <div v-if="data?.bodyLinks && data.bodyLinks.length > 0" class="mt-6 flex flex-wrap justify-center gap-4">
          <template v-for="link in data.bodyLinks" :key="link.id">
            <a v-if="link && link.link" :href="link.link"
              class="px-4 py-2 border border-blue-600 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-colors">
              {{ link.mainTitle }}
            </a>
          </template>
        </div>
    ...
    <section v-if="data?.ville && data.ville !== ''" class="featured portfolio bg-white dark:bg-gray-900 py-8">
      <div class="container max-w-6xl mx-auto px-4">
        <div class="sec-title">
    ...
    <section v-if="data?.page" class="featured portfolio bg-white dark:bg-gray-900 py-8">
      <div class="container max-w-6xl mx-auto px-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <template v-if="!data.page.secondTitle">
              <h2 class="text-2xl font-semibold mb-2">Référence en immobilier au Maroc</h2>
              <h3 class="text-xl font-semibold mb-4">Votre partenaire de confiance</h3>
              <p class="text-gray-600">
                Découvrez notre sélection de biens immobiliers à vendre au Maroc.
          ...
          <div class="flex justify-end items-center">
            <PageImage :page="data.page" :type-id="data.page?.type_id" :types-immobilier="data.page?.types_immobilier"
              default-image="/images/main_pages/Annonces-immobilier-a-vendre-au-Maroc.webp"
              :alt="data.page?.secondTitle || 'Immobilier à vendre Maroc'" />
          </div>
        </div>
      </div>
    </section>
    ...
    <FooterLinks v-if="data?.footerLinks && data.footerLinks.length > 0" :footer-links="data.footerLinks"
      :title="data?.footerTitle || ''" :category-id="data?.category_id || 1" :ville="data?.ville || ''"
...
const loadProducts = async () => {
  try {
    ...
    const response = await productService.getAchatProducts(params)
