<template>
  <div class="location">
    <!-- Hero Section -->
    <HeroSearch :title="pageTitle" :text="pageText" active="location" bg="location" :extras="data?.extras || {}"
      :extra="data?.extra || []" />
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <template v-if="!data.page.secondTitle">
              <h2 class="text-2xl font-semibold mb-2">Location immobilière au Maroc</h2>
              <h3 class="text-xl font-semibold mb-4">Votre partenaire de confiance</h3>
              <p class="text-gray-600">
                Découvrez notre sélection de biens immobiliers à louer au Maroc.
                Que vous cherchiez un appartement, une villa, un terrain ou un local commercial,
                nous vous accompagnons dans votre projet de location immobilière.
              </p>
            </template>
          </div>
          <div class="flex justify-end items-center">
            <PageImage :page="data.page" :type-id="data?.type_id" :types="data?.types || []"
              ...
              default-image="/images/main_pages/Annonces-immobilier-a-louer-au-Maroc.webp"
              :alt="data.page?.secondTitle || 'Location immobilière au Maroc'" />
          </div>
        </div>
      </div>
    </section>
    ...
    <FooterLinks
      v-if="data?.footerLinks && data.footerLinks.length > 0"
      :footer-links="data.footerLinks"
      :title="data?.footerTitle || ''"
      :category-id="data?.category_id || 2"
      :ville="data?.ville || ''"
      :type-id="data?.type_id"
      :extra="data?.extra || []"
      :footer-links-type="data?.footerLinksType || 'ville'"
    />
  </div>
</template>
...
const pageTitle = computed(() => {
  return data.value?.page?.mainTitle || 'Location Immobilière au Maroc'
})

const pageText = computed(() => {
  return data.value?.page?.mainText || 'Trouvez votre bien immobilier idéal parmi notre sélection de propriétés à louer au Maroc'
})
...
const loadProducts = async () => {
  try {
    ...
    const response = await productService.getLocationProducts(params)
