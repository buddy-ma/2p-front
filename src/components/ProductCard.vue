  ...
  <div
    ...
    <div class="relative w-full h-48">
      ...
      <div class="absolute inset-0 pointer-events-none">
        <!-- Left side badges - absolutely positioned -->
        <span v-if="product.vendu"
          class="absolute top-[3%] left-[15px] bg-red-500 text-white px-2 py-1 rounded text-xs font-semibold pointer-events-auto">
          {{ t('product.sold') }}
        </span>
        <span v-if="product.isPremium"
          class="absolute left-[15px] bg-red-500 text-white px-2 py-1 rounded text-xs font-bold shadow flex items-center gap-1 pointer-events-auto"
          :class="product.vendu ? 'top-[18%]' : 'top-[3%]'">
          <Tag class="w-3 h-3" />
          {{ t('product.premium') }}
        </span>
        <!-- Dynamic badges from product.badges array -->
        <span v-for="(badge, index) in product.badges" :key="badge.id"
          class="absolute left-[15px] px-2 py-1 rounded-[10px] text-xs font-semibold flex items-center gap-1 pointer-events-auto"
...
const getBadgeStyle = (badge, index) => {
  const bgColor = badgeColors[badge.color] || '#007bff'
  const textColor = ['warning', 'light'].includes(badge.color) ? '#333' : 'white'

  // Calculate top position: start after sold/premium badges if they exist
  let topOffset = 5
  if (props.product.vendu) topOffset += 15
  if (props.product.isPremium) topOffset += 15
  const top = topOffset + (index * 15) // Increment by 15% for each badge
