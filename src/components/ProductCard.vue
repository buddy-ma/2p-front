<template>
  <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow cursor-pointer" @click="goToProduct">
      ...
      <div class="absolute inset-0 flex justify-between items-start p-2 pointer-events-none">
        ...
        <div class="flex flex-col gap-2 pointer-events-auto">
          <span :class="`${colorClasses.bg} text-white px-2 py-1 rounded text-xs font-semibold`">
            {{ product.category?.title || 'Immobilier' }}
          </span>
        </div>
      </div>
